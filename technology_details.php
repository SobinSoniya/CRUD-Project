...
<!--start of header name-->
 <h1 class="h3 mb-4 text-gray-800"> Technology Known Setting</h1>
 <!--end of header name-->
        <?php
...
if(isset($_REQUEST['save']))
{
  ...
  //end of of varibale and value setting
	...
	$query = "INSERT INTO `technology_details`(`user_code`, `experience_type`, `experience_detail`) VALUES ('$user_code','$experience_type','$experience_detail')";  
...
if(isset($_REQUEST['edit'])){
  ...
  //end of of varibale and value setting
  //start of typing  query
	$query = "UPDATE `technology_details` SET `user_code`='$user_code',`experience_type`='$experience_type',`experience_detail`='$experience_detail' WHERE id='$id'";  
  // end of typing  query
	$message=iud_data($query);
  if($message=="sucess"){
		echo"<script>alert('data saved successfully ')</script>";
        ...
        $("body").on("click", "#btnExport", function () {
            html2canvas($('#example1')[0], {
                onrendered: function (canvas) {
                    ...
                    pdfMake.createPdf(docDefinition).download("technology_details.pdf");
                }
            });
        });
    </script>
